...
切换平台会立即按当前搜索条件查询，并返回第一页。设备编号、AppId、AppVersion、unionId 均为可选条件，可单独或组合查询（条件之间为“并且”）；留空表示不限制该字段。输入条件后点击“搜索”或按回车提交。翻页沿用已提交的条件，分享链接和从日志详情返回也会保留这些条件。

unionId 支持 `*` 和 `%` 通配符，均表示任意长度文本：
...
不带通配符时精确匹配；下划线 `_`、反斜杠 `\`、`+`、`&` 等按普通字符处理。其他文本筛选字段仍为精确匹配。分页、分享链接和详情返回保留原查询模式。需要更新后端才能启用模糊匹配，更新前端会显示输入提示；无需数据库迁移。
